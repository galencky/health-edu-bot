...
def get_user_session_sync(user_id: str) -> dict:
    """
    ...
    if not hasattr(get_user_session_sync, '_sync_lock'):
        get_user_session_sync._sync_lock = threading.Lock()
    
    with get_user_session_sync._sync_lock:
        # Update last access time
        _session_last_access[user_id] = datetime.now()
...
def get_user_session(user_id: str) -> dict:
    """
    Synchronous wrapper for get_user_session_sync.
    This maintains compatibility with existing synchronous code.
    """
    return get_user_session_sync(user_id)

def cleanup_expired_sessions():
    """
    Synchronous version of session cleanup for use in main.py
    """
    import threading
    
    # Use the same lock as get_user_session_sync
    if not hasattr(get_user_session_sync, '_sync_lock'):
        get_user_session_sync._sync_lock = threading.Lock()
